...
protected:
    virtual void do_log(std::ostream& stream, const Metadata& metadata, const std::string& message) const
    {
        ...
        if (pos != std::string::npos)
          result.replace(pos, 9, metadata.function ? (metadata.function.file + std::string(": [") + metadata.function.name + ":" + std::to_string(metadata.function.line) + std::string("]")) : "");
...
static std::ostream& operator<<(std::ostream& os, const Function& function)
{
    ...
    {
      os << function.file << ": " << function.name << "[" << function.line << "]";
    }
    return os;
}
